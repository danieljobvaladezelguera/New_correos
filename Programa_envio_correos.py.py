...
def inicio_sesion():
    ...
    with open(arch, "r") as dato:
        info = dato.read()
        array = info.split(",")
        nombre = array[0]#Usuario
    ...
    if fecha != fecha_hoy:
        ...
        for i in range(len(lineas)):# Separar cada línea por comas
            ...
            if columnas[0] == nombre:  # Buscar el usuario
                ...
                columnas[4] = str(fecha_hoy)  # Actualizar la fecha
    ...
    if nombre == '':#if cant_correo != 0 or nombre == '':
...
def cantidad_correos():
    ...
    info = Tk()
    info.geometry("450x75")
    info.title("Cantidad de correos")
